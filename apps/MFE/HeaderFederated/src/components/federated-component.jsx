...
function getServerComponent(ctx, remote, module, props) {
  ...
  if (!Component) {
    Component = ctx[id] = lazy(() =>
      ...
      axios
        .post(`${REMOTE_URLS[remote]}/prerender`, {
          headers: {
            ...
            default: ({children}) => {
              ...
              return (
                <>
                  ...
                  {chunks.map(chunk =>
                    chunk.endsWith('.css') ? (
                      <link key={chunk} rel="stylesheet" href={`${REMOTE_URLS[remote]}/build/${chunk}`} />
                    ) : (
                      <script key={chunk} async src={`${REMOTE_URLS[remote]}/build/${chunk}`} />
                    ),
                  )}
